...
$car = $cars_storage->findById($_GET['id']);
if (!$car) {
    echo "Car not found!";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   if (empty($brand) || empty($model) || empty($year) || empty($transmission) || empty($fuel_type) || empty($passengers) || empty($daily_price_huf) || empty($image)) {
       ...
       $car['model'] = $model;
       $car['year'] = (int)$year;
       $car['transmission'] = ucfirst(strtolower($transmission)); 
       $car['fuel_type'] = $fuel_type;
       $car['passengers'] = (int)$passengers;
       $car['daily_price_huf'] = (int)$daily_price_huf;
       $car['image'] = $image;

       $cars_storage->update($_GET['id'], $car);
...
<body>
    ...
    <main class="auth-form">
        ...
        <form method="post" novalidate>
            ...
            <input type="text" name="transmission" id="transmission" value="<?= htmlspecialchars($car['transmission']) ?>" required>
